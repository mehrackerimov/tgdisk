...
function getSafeFileName(name) {
    ...
    if (!safeName || safeName === "." || safeName === ".." || safeName === path.sep) {
        throw new Error("The file record does not contain a valid name.");
    }
    return safeName;
}
...
function validateFileEntry(fileEntry) {
    if (!fileEntry || !Array.isArray(fileEntry.parts) || fileEntry.parts.length === 0) {
        throw new Error("The file record or its part metadata is invalid.");
    }
    if (fileEntry.cipher && fileEntry.cipher !== "aes-256-gcm") {
        throw new Error(`Unsupported encryption: ${fileEntry.cipher}`);
    }
    if (fileEntry.encryptionFormat !== ENCRYPTION_FORMAT) {
        throw new Error("This is a legacy record with missing encryption metadata and cannot be downloaded safely.");
    }
    if (fileEntry.compressed === false) {
        throw new Error("Uncompressed file records are not supported.");
    }
}
...
async function downloadFile({ client, fileEntry, key, destinationDirectory = process.cwd(), onProgress }) {
    ...
    if (fs.existsSync(outputPath)) {
        throw new Error(`The destination file already exists: ${outputPath}`);
    }
    ...
    try {
        ...
        for (let index = 0; index < parts.length; index++) {
            ...
            if (!Number.isInteger(part.messageId)) {
                throw new Error(`Invalid Telegram message ID for part ${index + 1}.`);
            }

            const messages = await client.getMessages("me", { ids: part.messageId });
            const message = Array.isArray(messages) ? messages[0] : messages;
            if (!message?.media) {
                throw new Error(`Telegram message ${part.messageId} was not found or has no media.`);
            }

            const content = await client.downloadMedia(message.media);
            if (!Buffer.isBuffer(content)) {
                throw new Error(`Telegram part ${part.messageId} could not be downloaded.`);
            }
        ...
        if (stat.size <= IV_LENGTH + AUTH_TAG_LENGTH) {
            throw new Error("The encrypted data is corrupt or incomplete.");
        }
